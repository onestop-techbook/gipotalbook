...
ただ、この書き方では白色ボタンにも同じようなコードが並ぶ羽目になるため `.button` （いうなればボタン共通）と `.blackButton`（黒色ボタン） `.whiteButton`（白色ボタン）を切り分けるようにリファクタリングします。リファクタリングの基本は「前後で挙動が変わらない」ですので、つど見た目を確認しながら進めます。
