# Hasuraを構築する（モブワーク#12:2020/7/30）

連休やなんやらで少し空きましたが、今日はHasuraを構築し、実際にページとして表示します。

みんなで使えるGraphQLサーバーを立ててみましょう。

なお、Hasura.cloudというのもあるのですが、月99ドルからなので現実的ではありません。ローカルで立ててもデプロイの問題が出てくるので、Herokuでやってみます。

規模が難しいとか何か機能として無理になるとかの問題が生じたらその時考えることにしましょう。

## Herokuをみてみる
Herokuでやってみることにします。まずは、料金などの確認です。ギポタルでチームを作ります。5ユーザーまでは無料のチームが作れるようです。なお、クレカ登録が必要です。一旦お金はかからないけど、チーム人数が超えたときなどには請求される形だと考えられます。

とりあえず、今は個人IDで動作の練習とデプロイを確認してみることにしましょう。まずはIDを作って、ログインします。
...
あとは、Hasuraの画面からテストデータを作ります。Add new tableから模擬データを作ります。この辺りは、過去のモブでやったところですね。ところでここで、データタイプの型にName型というのがあることに気づき、なんじゃこりゃ、という感想も出ます。まあテキストでいいですよね。となりシンプルにテキストで進めます。

idとnameを適当に作って模擬データとします。たくさんありすぎても仕方ないので、二つ三つで十分です。

そして、GraphQLのURLをコピーして、VS Codeのindex.tsの中にある、URLを書き換えます。

Next.jsの画面を更新すると、データが読み込まれ、先ほど入力したデータが表示されれば成功です。

万全を期すためHasuraにデータを追加して、再度更新してみます。追加したデータを含めて表示されることを確認すれば万全です。
...
```
まずはお試しで上記のようにしてみます。ところこれでは動きません。Nextを再起動したところで。起動ログに.envが読み込まれている記載があったのでその点は問題ありません。

これが動かない理由はちゃんとあります。Next.jsはサーバーサイドにもクライアントサイドにも利用されるのですが、変更後の環境変数がクライアントに公開されてしまうことになります。そこで、環境変数をクライアントサイドに公開するのかしないのかを指定する必要があるのです。
...
これでは他に使ってみようと思った人が困ってしまいます。そこで、`.env.example`を作り、公開しておくとともにセットアップ方法をReadmeに記載します。初めてリポジトリを見た人にもわかるようにしておきます。
...
$ docker-compose up
# ここに一つ足りないものががある（Hasuraにダミーデータを入れる)
$ npm run dev
```
...
### リポジトリの名前を変える
ここまでの作業は見本的にGitHub上で実施していました。そのリポジトリはTech-book-portalという名前で進めていましたが、正式名称も決まったことですし、リポジトリ名自体を変えます。

```sh
git config -e
```

これで設定項目が開きますから、`[remote "origin"]`の中に記載されているURLを新しいリポジトリに変更します。viで開いてしまったため、viの使い方講座が始まったのは余談なのでここでは割愛します。
...
そこでHasuraが見ているPostgresSQLのマイグレーションのための調査、確認を行います。
...
```
Docker-composeを再起動しようとしたところ、Dockerがそもそも起動していないようだったので、Dockerをデーモンとして起動します。そして、Hasuraのコマンドラインツールをインストールします。あとは、初期化し、package.jsonに追記します。
...
Hasura関係でやっておきたいことが二つあり、それをついでにやってしまいます。

一つ目は、ApolloClientの初期化を個別ページでやっているので、それをindex.tsxから_app.tsxにお引越しします。

まず動作確認のために、サークルページにProfilesを表示してみます。
...
```
ApolloClienstが最新版じゃないため動かない？ということで、確認してみると、ちょうど現在のバージョンがメジャーアップの狭間だったようでした。このため、一度node_modulesとpackage_lock.jsonを消して、再インストールしてみたところ動きました。何もしてないのに壊れ、何もしてないのに直るという状況。これで動いたら笑うんだけど、と軽口をたたきながら、実際に動いてしまいます。いずれにせよ、こういうことができるのがやはりモブのいいところですね。しかもみんなで相談することで、いろいろな可能性や選択肢を見つけたりトライしてみたりすることができるのが素晴らしいですね。

これで、Hasuraで登録したデータを持ってくることができるようになりました。つまり、必要なデータをデータベースから持ってきて表示することができるようになったということです。

もう一つのやりたいことは、コードジェネレーターの追加です。

これは、package.jsonのdependanciesに追加します。

また、今の状態ではクエリでHasuraからデータを取ってくるときに、せっかく型のある言語なのに型が付きません。そこで、TypeScriptの型情報を吐き出す際などに必要なツール群を追加します。
...
```
generatedというフォルダできており、さらにその中にgraphql.tsというファイルができています。
...
この変更により、.graphqlというファイルにクエリを外だしすることができ、かつそれをアクセスするための関数(useGetProfileQuery)が自動で生成されたことになります。結果としてComponentとQueryの分割ができ、見通し、管理がしやすくなります。利用する側からは、関連するクエリを使えるようになりますし、クエリだけを作っていくことでそれを読み取り配置していくだけにすることも可能になります。
...
```
またサークル情報を取るクエリは、引数を増やし、上記のように書くことができます。
...
一旦、graphqlコード生成(/src/generated)は.gitignoreに追加しておき、GitHubへpushされないようにします。
...
Hasuraからデータがとってこれるようになったので、今後はフロントとデータベースをがつがつ進めていけるようになりました。
...
![7月30日のFun done learn](chap-mob-0730/0730fundonelearn.png?scale=1.0)
